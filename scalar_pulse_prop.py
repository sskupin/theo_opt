...
def calculate():
    ...
    lns1 = a3.plot(np.abs(u[-1,:])/np.max(np.abs(u0)),eta,'b', label=r'$|v(z,\tau)|$')
    ...
    lns2 = a3bis.plot(np.abs(U)/np.max(np.abs(U0)),keta,'r', label=r'$|V(z,\overline\omega)|$')
...
row = gui.create_slider_with_latex(mainframe,r'Normalized GVD $k_0^{(2)}L/T_0^2=$',k2_double,-2,2,row)
row = gui.create_slider_with_latex(mainframe,r'Normalized TOD $k_0^{(3)}L/T_0^3=$',k3_double,-2,2,row)
row = gui.create_slider_with_latex(mainframe,r'Normalized linear losses $k_0''L=$',kloss_double,0,1,row)
