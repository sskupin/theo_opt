...
def calculate():
    ...
    a4.cla()      
    a4.plot(np.fft.fftshift(kx)/k,(np.abs(np.fft.fftshift(V))*foufac)**2,'b')
    a4.set_xlim([-2/b,2/b])
    a4.set_xlabel(r'$k_x/k$')
    a4.set_ylabel(r'$|U_-(z=f)|^2$ [norm. u.]')
    ...
    a5.cla()
    a5.plot(x/f,np.abs(v)**2,'b')
    a5.set_xlim([-xlim,xlim])
    a5.set_ylim([-np.max((np.abs(v))**2)*0.05,np.max((np.abs(v))**2)*1.15])
    if AL/(f)<xlim:
        if AL/(f)>0.04*xlim:
           a5.annotate(r'2A', xy=(0,np.max((np.abs(v))**2)*1.075),horizontalalignment='center', verticalalignment='center')
        if AL/(f)>0.1*xlim:
           a5.annotate(r'', xy=(-AL/(f),np.max((np.abs(v))**2)*1.075), xytext=(-0.05*xlim,np.max((np.abs(v))**2)*1.075), arrowprops=dict(arrowstyle='->'))
           a5.annotate(r'', xy=(AL/(f),np.max((np.abs(v))**2)*1.075), xytext=(0.05*xlim,np.max((np.abs(v))**2)*1.075), arrowprops=dict(arrowstyle='->'))
    a5.plot([-AL/f,-AL/f],a5.get_ylim(),'k:',[AL/f,AL/f],a5.get_ylim(),'k:')
    a5.set_xlabel(r'$x/f$')
    a5.set_ylabel(r'$|u_+(z=f)|^2$ [norm. u.]')
    
    V = np.fft.fft(v)
    a6.cla()      
    a6.plot(np.fft.fftshift(kx)/k,(np.abs(np.fft.fftshift(V))*foufac)**2,'b')
    a6.set_xlim([-3*w/f,3*w/f])
    a6.set_xlabel(r'$k_x/k$')
    a6.set_ylabel(r'$|U_+(z=f)|^2$ [norm. u.]')
    ...
    v = np.fft.ifft(V)
    a7.cla() 
    a7.plot(x/f,np.abs(v)**2,'b')
    a7.set_xlim([-xlim,xlim])
    a7.set_xlabel(r'$x/f$')
    a7.set_ylabel(r'$|u(z=2f)|^2$ [norm. u.]')
    
    V = np.fft.fft(v)
    a8.cla()      
    a8.plot(np.fft.fftshift(kx)/k,(np.abs(np.fft.fftshift(V))*foufac)**2,'b')
    a8.set_xlim([-3*w/f,3*w/f])
    a8.set_xlabel(r'$k_x/k$')
    a8.set_ylabel(r'$|U(z=2f)|^2$ [norm. u.]')
